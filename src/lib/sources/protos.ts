...
export const protos: SourceAdapter = {
  source: "protos",
  label: "Protos",
  async fetch(): Promise<RawArticle[]> {
    const feed = await parser.parseURL("https://protos.com/feed/");
    ...
    for (const item of items) {
      ...
      out.push({
        title,
        url,
        imageUrl: pickImage(item as Record<string, unknown>, { baseUrl: "https://protos.com" }),
