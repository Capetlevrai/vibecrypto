...
export const therage: SourceAdapter = {
  source: "therage",
  label: "The Rage",
  async fetch(): Promise<RawArticle[]> {
    const feed = await parser.parseURL("https://www.therage.co/rss/");
    ...
    for (const item of items) {
      ...
      out.push({
        title,
        url,
        imageUrl: pickImage(item as Record<string, unknown>, { baseUrl: "https://www.therage.co" }),
